...
public class fig4 {
	public  void main(String login, String senha,String local,String diretorio,List<Integer> pracas) {
		...
		Conecta.setSql( "select day(id_data_hora) as dia,month(id_data_hora) as mes,year(id_data_hora)  as ano,date_format(id_data_hora,'%Y-%m-%d') as DataCompleta, id_praca,sum(usuarios) , format(sum(entrada)/1024,2,'de_DE') as entrada from dados_praca_por_hora "+
				"where "+ pesquisa+
		...
		try {
			...
			try {
				StrW = new BufferedWriter(new FileWriter(diretorio+"\\fig4.csv"));
			} catch (IOException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
			StrW.write("dia ; mes ; ano ; data completa ; praca ; soma usuarios ; entrada total em Mbits \n");
			while(rs3.next()){
