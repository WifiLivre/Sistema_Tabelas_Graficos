...
public class fig13 {
	...
	public  void main(String login, String senha,String local,String diretorio,List<Integer> pracas) {
		...
		Conecta.setSql("select DATE_FORMAT(id_hora,'%H') as hora , id_praca , format(avg(numeroUsuarios),2,'de_DE') as mediaUsuarios,format(avg(latencia),2,'de_DE')  as mediaLatencia "+
		...
		try {
			...
			try {
				StrW = new BufferedWriter(new FileWriter(diretorio+"\\fig13.csv"));
